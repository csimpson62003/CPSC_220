	...
	if (enemyCount == 0) {
		...
		for (int i = 0; i < 5 * this->getWorldLevel(); i++) {
			...
			case 1: //Enemy will spawn in on the left side
				addEnemy(1 + (16 * scale), GetRandomValue(1 + (16 * scale), worldHeight - (16 * scale) - 1), 16, 16, enemy, "", enemyBaseHealth);
				break;
			case 2: //Enemy will spawn on the right side
				addEnemy(worldWidth - (16 * scale) - 1, GetRandomValue(1 + (16 * scale), worldHeight - (16 * scale) - 1), 16, 16, enemy, "", enemyBaseHealth);
				break;
			case 3: //Enemy will spawn in on the top
				addEnemy(GetRandomValue(1 + (16 * scale), worldWidth - (16 * scale) - 1), 1 + (16 * scale), 16, 16, enemy, "", enemyBaseHealth);
				break;
			case 4: //Enemy will spawn on the bottom
				addEnemy(GetRandomValue(1 + (16 * scale), worldWidth - (16 * scale) - 1), worldHeight - (16 * scale) - 1, 16, 16, enemy, "", enemyBaseHealth);
				break;
			}
		};
